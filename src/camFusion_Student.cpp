...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    distance = distance / n;
    for(cv::DMatch& match : kptMatches)
        if(boundingBox.roi.contains(kptsCurr[match.trainIdx].pt) && match.distance < distance*1.2)
                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC, cv::Mat *visImg)
{
    ...
    {
        cv::KeyPoint curr1 = kptsCurr.at(it1->trainIdx);
        cv::KeyPoint prev1 = kptsPrev.at(it1->queryIdx);
        for (auto it2 = kptMatches.begin() + 1; it2 != kptMatches.end(); ++it2)
        {
            double minDist = 100.0; // min. required distance

            cv::KeyPoint curr2 = kptsCurr.at(it2->trainIdx);
            cv::KeyPoint prev2 = kptsPrev.at(it2->queryIdx);

            double distCurr = cv::norm(curr1.pt - curr2.pt);
            double distPrev = cv::norm(prev1.pt - prev2.pt);
            if (distPrev > 0.00001 && distCurr >= minDist)
            {
...
{   
    int counts[prevFrame.boundingBoxes.size()][currFrame.boundingBoxes.size()] = { };
    for (auto it = matches.begin(); it != matches.end() - 1; ++it)     
    {
        cv::KeyPoint prev = prevFrame.keypoints[it->queryIdx];
        auto prevPt = cv::Point(prev.pt.x, prev.pt.y);

        cv::KeyPoint curr = currFrame.keypoints[it->trainIdx];
        auto currPt = cv::Point(curr.pt.x, curr.pt.y);
